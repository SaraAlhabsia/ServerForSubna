...
import AlhabsiModel from "./Models/Alhabsi.js";
import AlrajulModel from "./Models/Alrajul.js";
import BroseModel from "./Models/Brose.js";
...
app.get("/Alhabsi", async (req, res) => {
  try {
    const products = await AlhabsiModel.find();
    res.json(products);
  } catch (err) {
...
app.get("/Alrajul", async (req, res) => {
  try {
    const products = await AlrajulModel.find();
    res.json(products);
  } catch (err) {
    res.status(500).send(err);
  }
...
app.get("/Brose", async (req, res) => {
  try {
    const products = await BroseModel.find();
    res.json(products);
  } catch (err) {
    res.status(500).send(err);
  }
...
var conn =
  "mongodb+srv://admin:1234@cluster0.fjfgp1h.mongodb.net/SubnaDB?retryWrites=true&w=majority&appName=Cluster0";
